...
export default async request => {
  ...
  if (request.method === 'PUT') {
    ...
    const record = {
      ...
      photoConsent: typeof body?.photoConsent === 'boolean' ? body.photoConsent : previous.photoConsent === true,
      photoConsentRecorded: body?.photoConsentRecorded === true || previous.photoConsentRecorded === true,
      consentTermsVersion: Math.max(0, Number(body?.consentTermsVersion ?? previous.consentTermsVersion ?? 0)),
