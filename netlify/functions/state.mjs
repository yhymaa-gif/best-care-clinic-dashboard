...
export default async request=>{
 ...
 if(request.method==='PUT'||request.method==='POST'){let body;try{body=await request.json()}catch{return reply({error:'Invalid JSON'},400)}if(!Array.isArray(body.patients)||body.patients.length>300)return reply({error:'Invalid patients'},400);const existing=await store.get(key,{type:'json',consistency:'strong'});const state={date,patients:body.patients.map(p=>({id:String(p.id||'').slice(0,80),name:String(p.name||'').slice(0,80),file:String(p.file||'').slice(0,40),start:String(p.start||'').slice(0,8),end:String(p.end||'').slice(0,8),procedure:String(p.procedure||'').slice(0,180),status:['waiting','active','done','late','cancel'].includes(p.status)?p.status:'waiting'})),notes:String(body.notes||'').slice(0,5000),updateAlert:cleanAlert(body.updateAlert),clientId:String(body.clientId||'').slice(0,100),revision:Number(existing?.revision||0)+1,updatedAt:Date.now()};await store.setJSON(key,state);return reply({ok:true,revision:state.revision,updatedAt:state.updatedAt})}
