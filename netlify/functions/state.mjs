...
const cleanPatient=p=>({
 ...
 recordUpdatedAt:Number(p?.recordUpdatedAt||p?.adminUpdatedAt||0),
  ...
  actualEndedAt:Number(p?.actualEndedAt||0),
...
export default async request=>{
 ...
 const store=getStore({name:'clinic-dashboard-days',consistency:'strong'}),key=clinicId==='clinic-1'?`days/${date}`:`clinics/${clinicId}/days/${date}`;
  if(request.method==='GET'){const state=await store.get(key,{type:'json',consistency:'strong'});if(!state)return reply({exists:false,date,patients:[],notes:'',updateAlert:cleanAlert(null),revision:0,updatedAt:0});const knownRevisionParam=url.searchParams.get('knownRevision'),knownRevision=knownRevisionParam===null?NaN:Number(knownRevisionParam);if(Number.isFinite(knownRevision)&&knownRevision>=0&&knownRevision===Number(state.revision||0))return reply({exists:true,changed:false,revision:Number(state.revision||0),updatedAt:Number(state.updatedAt||0)});if(auth.user?.role!=='admin')return reply({exists:true,...state,changed:true,updateAlert:cleanAlert(state.updateAlert)});const patientDirectory=await getPatientDirectory();const patients=Array.isArray(state.patients)?state.patients.map(patient=>enrichPatientFromDirectory(patientDirectory,patient)):[];return reply({exists:true,...state,changed:true,patients,updateAlert:cleanAlert(state.updateAlert)})}
