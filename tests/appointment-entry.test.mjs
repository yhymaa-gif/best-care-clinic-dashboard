...
test('new appointment page uses central lookup and revision-safe shared day state',async()=>{
  ...
  assert.match(serviceWorker,/appointment-entry\.html/);assert.match(serviceWorker,/20260906-clear-status-notifications/);assert.match(manifest,/appointment-entry\.html\?source=pwa/);
});
